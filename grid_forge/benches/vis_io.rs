...
use bencher::Bencher;
use grid_forge::{tile::{identifiable::{builder::IdentTileTraitBuilder, BasicIdentifiableTile2D}, vis::DefaultVisPixel}, vis::{collection::VisCollection, ops::{init_map_image_buffer, load_gridmap_identifiable_auto, load_gridmap_identifiable_manual, write_gridmap_identifiable}}};

fn load_gridmap_auto(bencher: &mut Bencher) {

  let builder = IdentTileTraitBuilder::<BasicIdentifiableTile2D>::default();
  let image = image::open("../assets/samples/roads.png").unwrap().into_rgb8();
...
benchmark_group!(benches, load_gridmap_auto, load_gridmap_manual, write_grimap_ident);
