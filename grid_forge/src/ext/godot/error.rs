...
use super::collection::GodotTileInfo;
...
impl GodotTileError {
    pub fn new_no_id_for_info(tile_info: GodotTileInfo) -> Self {
        Self {
...
impl Display for GodotTileError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match &self.kind {
            GodotTileErrorKind::NoTileForInfo(info) => write!(f, "cannot get `tile_type_id` for tile info: {info:?}"),
            GodotTileErrorKind::NoInfoForTile(id) => write!(f, "cannot get `GodotTileInfo` for `tile_type_id`: {id}"),
