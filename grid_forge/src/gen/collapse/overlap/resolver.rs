    ...
    Data: IdentifiableTileData,
{
        ...
        Queue: CollapseQueue,
    {
        ...
        // let position_to_check = GridPosition::new_xy(0, 8);
        // let pattern_to_check = grid.option_data.get_tile_data(&12517753753361199696).unwrap();
        // println!("{pattern_to_check}");

        while let Some(collapse_position) = queue.get_next_position() {

          // if collapse_position == position_to_check {
          //   println!("got it!");
          // }
...
impl Subscriber for DebugSubscriber {
  fn on_collapse(&mut self, position: &GridPosition, tile_type_id: u64, pattern_id: u64) {
      println!("tile_type_id: {tile_type_id}, pattern_id: {pattern_id} on position: {position:?}");
  }
}