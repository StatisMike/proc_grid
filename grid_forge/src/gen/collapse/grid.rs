...
use crate::{
    ...
    tile::{
        identifiable::{builders::IdentTileBuilder, IdentifiableTileData}, GridPosition, GridTile, GridTileRef, TileContainer
    },
};
    ...
    Sized + private::Sealed<CT>
{
    ...
    fn retrieve_positions(&self, collapsed: bool) -> Vec<GridPosition> {
        ...
        self
            ._grid()
            .iter_tiles()
            .filter_map(|t| if func(&t) { Some(t.grid_position()) } else { None })
            .collect()
    }
