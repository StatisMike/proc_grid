use crate::{map::{GridDir, GridMap2D}, tile::GridTile2D};

use std::{collections::{BTreeMap, BTreeSet, HashMap}, hash::{DefaultHasher, Hash, Hasher}};
...
impl WFCNeighbours {
  ...
  pub fn is_valid_at_dir(&self, dir: &GridDir, option: u64) -> bool {
    self.inner.get(dir).expect("no valid tiles at direction").contains(&option)
  }
}
