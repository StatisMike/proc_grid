...
use crate::{map::{GridDir, GridMap2D}, GridPos2D};
...
where T: WFCTile
{
  ...
  pub fn is_valid_at_dir(&self, option: u64, neighbour_option: u64, dir: &GridDir) -> bool {
    self.neigbours.get(&option).unwrap().is_valid_at_dir(dir, neighbour_option)
  }
...
impl WFCTileProbs {
  ...
  pub fn tiles_by_prob(&self) -> Vec<u64> {
    ...
    tiles_by_prob.iter().map(|x| x.0).copied().collect::<Vec<_>>()
  }
...
#[cfg(test)]
pub (crate) mod test {
  use crate::{gen::wfc::WFCTile, map::{GridMap2D, GridSize}, tile::GridTile2D, GridPos2D};
  ...
  pub (crate) fn get_test_map() -> TestMap {
    let mut tiles = [
      [
        TT::new(0), TT::new(0), TT::new(1), TT::new(1), TT::new(0), TT::new(0)
      ],
      [
        TT::new(0), TT::new(0), TT::new(1), TT::new(1), TT::new(0), TT::new(0)
      ],
      [
        TT::new(0), TT::new(1), TT::new(1), TT::new(1), TT::new(0), TT::new(0)
      ],
      [
        TT::new(1), TT::new(1), TT::new(2), TT::new(2), TT::new(0), TT::new(1)
      ],
      [
        TT::new(1), TT::new(2), TT::new(3), TT::new(2), TT::new(1), TT::new(1)
      ],
      [
        TT::new(0), TT::new(1), TT::new(3), TT::new(1), TT::new(1), TT::new(1)
      ]
    ];
  ...
  fn can_generate_probs() {
    ...
    let higher_entrophy = probs.total_entropy(&[tiles_by_probs.first().unwrap().0, tiles_by_probs.get(1).unwrap().0]);
    let lower_entrophy = probs.total_entropy(&[tiles_by_probs.first().unwrap().0, tiles_by_probs.last().unwrap().0]);
