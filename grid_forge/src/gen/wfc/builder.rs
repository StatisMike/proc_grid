...
where T: IdentifiableTile + Clone
{
...
where T: IdentifiableTile + Clone
{
    fn create_wfc_tile(&self, pos: GridPos2D, tile_id: u64) -> T {
        let mut tile = self.tiles.get(&tile_id).unwrap_or_else(|| panic!("can't get tile with tile id: {tile_id}")).clone();
...
{
    fn create_wfc_tile(&self, pos: GridPos2D, tile_id: u64) -> T {
        let fun = self.funs.get(&tile_id).unwrap_or_else(|| panic!("can't get tile function with `wfc_id`: {tile_id}"));

        fun(pos, tile_id)
    }

    fn missing_tile_creators(&self, analyzer: &WFCAnalyzer<T>) -> Vec<u64> {
      analyzer.tiles().iter().filter(|tile_id| !self.funs.contains_key(tile_id)).copied().collect::<Vec<u64>>()
    }
}

pub trait WFCConstructTile
where Self: IdentifiableTile
{
